        ...
        <flux:sidebar.nav>
            @foreach (Auth::user()->menu as $page)
                @if (!empty($page['childrens']))
                    <flux:sidebar.group expandable :heading="$page['label']" class="grid" :icon="$page['icon']"
                ...
                <flux:menu.radio.group>
                    <div class="p-0 text-sm font-normal">
                        <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
                            <flux:avatar :name="auth()->user()?->user_information->nombre_corto"
                                :initials="auth()->user()->initials()" />

                            <div class="grid flex-1 text-start text-sm leading-tight">
                                <flux:heading class="truncate">{{ auth()->user()?->user_information->nombre_corto }}
                                </flux:heading>
